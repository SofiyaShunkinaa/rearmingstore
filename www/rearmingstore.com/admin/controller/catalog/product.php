...
class ControllerCatalogProduct extends Controller {
    ...
    public function add(): void {
        ...
        // Products
        $this->load->model('catalog/product');
// 		$products = $this->model_catalog_product->getRelated(42);
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $product_id = $this->model_catalog_product->addProduct($this->request->post);
			$this->model_catalog_product->copyAddProductServ($product_id);
    ...
    public function copy(): void {
        ...
        if (isset($this->request->post['selected']) && $this->validateCopy()) {
            foreach ((array)$this->request->post['selected'] as $product_id) {
                $product_id_new = $this->model_catalog_product->copyProduct($product_id);
				$this->model_catalog_product->copyAddProductServ($product_id_new);
            }
    ...
    protected function getList() {
        ...
        foreach ($results as $result) {
			...
			$products = $this->model_catalog_product->getRelated($result['product_id']);
			$us = '';
			if($products){
				foreach($products as $p){
					$us = $this->url->link('catalog/product/edit', 'user_token=' . $this->session->data['user_token'] . '&product_id=' . $p . $url, true);
				}
			}
    ...
    protected function getForm() {
		...
		if(!isset($data['product_description'][1]['tag'])||empty($data['product_description'][1]['tag'])){
