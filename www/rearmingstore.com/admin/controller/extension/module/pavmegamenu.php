...
class ControllerExtensionModulePavmegamenu extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')  && !empty($this->request->post) ) {
			if (!$this->user->hasPermission('modify', 'extension/module/'.$this->moduleName)) {
                ...
                $store_param = isset($megamenu['store'])?'&store='.$megamenu['store']:'';
		...
		if (isset($this->error['dimension'])) {
  		...
  		$this->mdata['breadcrumbs'] = array();
   		...
   		$updateTree = $this->url->link('extension/module/pavmegamenu/update', 'root=0'.$store_param.'&user_token=' . $this->session->data['user_token'], true);
		...
		$this->mdata['liveedit_url']  = $this->url->link('extension/module/pavmegamenu/liveedit', 'root=0'.$store_param.'&user_token=' . $this->session->data['user_token'], true);
		...
		$this->response->setOutput($this->load->view('extension/module/pavmegamenu/pavmegamenu', $data));
	}
	...
	public function delete(){
		...
		if( isset($this->request->get['id']) ){
			...
			$store = ($store_id == 0)?'':'&store='.$store_id;
	...
	public function info(){
		...
		//$this->template = 
        $this->response->setOutput($this->load->view('extension/module/pavmegamenu/pavmegamenu_form', $this->mdata));
		//$this->response->setOutput($this->render());
