...
class ModelExtensionMenuMegamenu extends Model {
	 ...
	 */
	public function getTree( $id=null, $store_id = 0 , $selected=0 ){
	 ...
	 */
	public function genOption( $parent, $level=0, $selected=0){
		$output = '';
		if( $this->hasChild($parent) ){
			$data = $this->getNodes( $parent );
	...
	public function importCategories($store_id = 0){
		...
		foreach ($categories as &$category){
			$category['language'] = $this->model_catalog_category->getDescriptions($category['category_id']);
			...
			if($this->checkExitItemMenu($category, $store_id) == 0){
				...
				$this->insertCategory($megamenu_parent_id,$category,  $store_id);
			}
		}
	}
	public function insertCategory($megamenu_parent_id, $category = array(),  $store_id = 0){
		$data = array();
