...
class ModelCatalogProduct extends Model {
    public function addProduct(array $data): int {
        ...
        foreach ($data['product_description'] as $language_id => $value) {
            $this->db->query("INSERT INTO `" . DB_PREFIX . "product_description` SET `product_id` = '" . (int)$product_id . "', `language_id` = '" . (int)$language_id . "', `name` = '" . $this->db->escape($value['name']) . "', `description` = '" . $this->db->escape($value['description']) . "', `timestart` = '" . $this->db->escape($value['timestart']) . "', `timecomplete` = '" . $this->db->escape($value['timecomplete']) . "', `why` = '" . $this->db->escape($value['why']) . "', `payments` = '" . $this->db->escape($value['payments']) . "', `tag` = '" . $this->db->escape($value['tag']) . "', `meta_title` = '" . $this->db->escape($value['meta_title']) . "', `meta_description` = '" . $this->db->escape($value['meta_description']) . "', `meta_keyword` = '" . $this->db->escape($value['meta_keyword']) . "'");
        }
    ...
    public function editProductOption(int $product_id, array $data): void {
            try{
    ...
    public function copyProduct(int $product_id) {
        ...
        if ($query->num_rows) {
            ...
            $p_id = $this->addProduct($data);
			return $p_id;
        }
    }
	
	public function copyAddProductServ(int $product_id) {
        $query = $this->db->query("SELECT DISTINCT * FROM `" . DB_PREFIX . "product` `p` WHERE `p`.`product_id` = '" . (int)$product_id . "'");
        if ($query->num_rows) {
            $data = $query->row;
            ...
            $data['product_image'] = $this->getImages($product_id);
            $data['product_option'] = $this->getOptions($product_id);
            $data['product_related'] = array($product_id);
            $data['product_reward'] = $this->getRewards($product_id);
            $data['product_special'] = $this->getSpecials($product_id);
            $data['product_category'] = $this->getCategories($product_id);
            $data['product_download'] = $this->getDownloads($product_id);
            ...
            $data['product_store'] = $this->getStores($product_id);
            $data['product_subscriptions'] = $this->getSubscriptions($product_id);
			if(isset($data['product_description'][1]['tag'])){
				$data['product_description'][1]['tag'] = 'US SERVERS';
			}
			/*
    ...
    public function test(int $product_id): array {
    ...
    public function getDescriptions(int $product_id): array {
        ...
        foreach ($query->rows as $result) {
            $product_description_data[$result['language_id']] = [
                ...
                'timecomplete'     => $result['timecomplete'],
                'why'              => $result['why'],
