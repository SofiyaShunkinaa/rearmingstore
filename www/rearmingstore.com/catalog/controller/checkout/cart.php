...
class ControllerCheckoutCart extends Controller {
    ...
    public function ajaxUpdate() {
		...
		if (isset($this->request->post['quantity']) && isset($this->request->post['key'])) {
			...
			// Пересчет суммы товара и общей суммы
			$product_total = 0;
			$total = 0;
			...
			$json['total'] = 'Total: '.$this->currency->format($total, $this->session->data['currency']);
			$json['success'] = 'Product quantity updated';
		} else {
			$json['error'] = 'Required data missing';
		}
