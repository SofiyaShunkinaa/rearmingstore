    ...
    public function getProducts($products, $currency){
        try {
            ...
            foreach ($products as $p) {
                ...
                $description = isset($product_info['description']) ? htmlspecialchars_decode($product_info['description']) : '';
                array_push(
                    $products_array,
                    array( 
                        'price' => isset($product_info['price'])  ? $product_info['price'] : 0,
                        'categories' => $this->getProductCategories($product_info['product_id']),
                        'description' => $this->stripAllTags($description, true),
                        'images' => $this->getAllImages($product_info['product_id'], $image_width, $image_height),
                        'tags' => $product_info['tag'] ? explode(',', $product_info['tag']) : array(),
                        'meta' => array(
        ...
        foreach ($categories as $category) {
            if ($category) {
                ...
                if (isset($category_info['name'])) {
                    array_push($categoryNames, html_entity_decode($category_info['name']));
                }
            }
        }
