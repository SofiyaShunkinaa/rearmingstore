...
class ModelAccountCustomer extends Model {
	...
	public function editCustomer($customer_id, $data) {
        // Обновление основных полей в таблице customer
		...
		if(!isset($custom_fields[1])){$custom_fields = array(); $custom_fields[1] = '';}
		// Обновление 'discord', если передано новое значение
		if (isset($data['discord']) && $data['discord'] != $custom_fields[1]) {
			$custom_fields[1] = $data['discord'];

			// Кодирование в JSON и обновление в базе данных
			$custom_field_json = json_encode($custom_fields);
			$this->db->query("UPDATE " . DB_PREFIX . "customer SET custom_field = '" . $this->db->escape($custom_field_json) . "' WHERE customer_id = '" . (int)$customer_id . "'");
		}
    }
	public function checkPassword($customer_id, $password) {
		$query = $this->db->query("SELECT password FROM " . DB_PREFIX . "customer WHERE customer_id = '" . (int)$customer_id . "'");
