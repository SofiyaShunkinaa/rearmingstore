...
class __TwigTemplate_20a094483e58589ae4593fc9ae915b8d extends Template
{
    private $source;
    private $macros = [];
...
\t\t\t\$(\"#sliderValue\").val(data.from);  
\t\t},
\t\tonChange: function (data) {
\t\t\t\$(\"#sliderValue\").val(data.from); 
\t\t\tonChange: function (data) {
            const sliderValue = \$('#sliderValue');
            sliderValue.text(data.from); // Обновляем текстовое значение

            // Ваши вычисления и выводы
            if (document.querySelector('.titlepage').textContent == \"WoW Solo Shuffle Wins Boost\") {
                var total = data.from * 2;
