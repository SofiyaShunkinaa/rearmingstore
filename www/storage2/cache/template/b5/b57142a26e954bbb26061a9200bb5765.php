...
class __TwigTemplate_c23e3a0a0935bd8682d7862ca3e69101 extends Template
{
    private $source;
    private $macros = [];
...
\tfunction updateInfoText(optionName) {
\t\tif (optionName.includes(\"(x3 + 1 free)\")) {
\t\t\$('#info-text').text(\"3 hours order completion\");
\t\t} else if (optionName.includes(\"(x6 + 2 free)\")) {
\t\t\$('#info-text').text(\"6 hours order completion\");
\t\t} else {
\t\t\$('#info-text').text(\"3 hours order completion\"); 
\t\t}
\t}
\t
\t\$(document).on('click', '.scrollbar-inner .dropdown-item', function(event) {
\t\tevent.preventDefault(); 
\t\tconsole.log('Function is called'); // Проверяем, вызывается ли функция

\t\tvar optionName = \$(this).data('name'); // Используем data-атрибут для получения имени
\t\tconsole.log(\"Option name: \" + optionName); // Проверяем имя выбранного элемента

\t\tupdateInfoText(optionName); // Обновляем текст
\t});


\tconsole.log(\$('.dropdown-item').length)
</script>
