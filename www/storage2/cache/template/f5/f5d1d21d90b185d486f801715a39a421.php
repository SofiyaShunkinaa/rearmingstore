...
class __TwigTemplate_483ddc9ef656c79d200be48ccce1aa62 extends Template
{
    private $source;
    private $macros = [];
...
document.addEventListener(\"DOMContentLoaded\", function() {
    // Итерируем по всем элементам с классом 'dropdown-item'
    document.querySelectorAll('.dropdown-item').forEach(function(item) {
        // Проверяем, равняется ли атрибут data-value \"Original Price\"
        if (item.getAttribute('data-value') === 'Original Price') {
            // Находим родительский элемент с классом 'row' и скрываем его
...
\t\t\t\t} 
\t\t\t}else{
\t\t\t\t\tconsole.log(\"we are here: \"+values.length)
...
\t\t\t\t\tbreak;
\t\t\t\tcase [\"Keystone Hero Boost\", \"Mythic+ Gear Boost\", \"Mythic+ BiS Items Boost\"].includes(productName):
\t\t\t\t    calcTimeBySpecialOptions(checkedSpecialValues);
\t\t\t\t\tbreak;\t
\t\t\t\tcase [\"Mythic+ BiS Items Boost\"].includes(productName):
\t\t\t\t    calcTimeBySpecialOptions(checkedlValues);
\t\t\t\t\tbreak;\t
\t\t\t}
\t\t
\t});
