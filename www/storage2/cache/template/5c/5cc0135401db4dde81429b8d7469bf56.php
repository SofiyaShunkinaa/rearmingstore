...
class __TwigTemplate_93abde939096c46f53c34e6ce5a3867e extends Template
{
    private $source;
    private $macros = [];
...
document.addEventListener(\"DOMContentLoaded\", function() {
    function hideRowIfOriginalPrice() {
        document.querySelectorAll('.dropdown-item').forEach(function(item) {
            if (item.getAttribute('data-value') === 'Original Price') {
                const rowElement = item.closest('.row');
                if (rowElement) {
    ...
    // Изначально проверяем, нужно ли скрыть блоки
    hideRowIfOriginalPrice();

    // Создаем MutationObserver для отслеживания изменений в DOM
    const observer = new MutationObserver(hideRowIfOriginalPrice);

    // Настраиваем observer для всего документа
