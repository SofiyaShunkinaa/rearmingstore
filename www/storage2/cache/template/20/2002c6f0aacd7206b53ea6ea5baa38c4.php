...
class __TwigTemplate_24e3411c9912221f196d0334b8f350e5 extends Template
{
    private $source;
    private $macros = [];
...
\t\tconst extra_days = Math.floor(totalHours / 24);
\t\tconst extra_hours = Math.floor(totalHours % 24);
\t\tconst extra_minutes = Math.round((totalHours % 1) * 60);
...
\t\tconst hours = parseInt(\$('#comlete-time-text').attr('data-hours')) || 0; // Получаем часы из элемента
\t\tconst minutes = parseInt(\$('#comlete-time-text').attr('data-minutes')) || 0; // Получаем минуты из элемента

\t\t// Сложение значений
\t\tconst totalDays = extra_days; // Дни остаются только из вычислений
\t\tlet totalHours = extra_hours + hours; // Суммируем часы
\t\tlet totalMinutes = extra_minutes + minutes; // Суммируем минуты
...
\t\t// Формируем вывод
\t\tlet output = `\${totalDays} day(s)`;
\t\tif (totalHours > 0) {
\t\t\toutput += `, \${totalHours} hour(s)`;
\t\t} else if (totalMinutes > 0) {
\t\t\toutput += `, \${totalMinutes} minute(s)`;
\t\t}

\t\tconsole.log(`Total time: \${output}`);
\t}
...
\t\t\tvar text = \t(days > 0 ? \" \" + (days === 1 ? days + \" day \" : days + \" days \") : \"\") +
\t\t\t\t\t(hours === 0 ? \"\" : (hours + \" hour\" + (hours !== 1 ? \"s\" : \"\")));
\t\t\tif (minutes>0){
