...
class __TwigTemplate_2e9570b3b3b0a9edfd9058748c5270aa extends Template
{
    private $source;
    private $macros = [];
...
\t\ttimeString = \$('#comlete-time-text').text();
\t\tconst timeUnits = { day: 0, hour: 0, minute: 0 };
...
\t\t\t.filter(([_, value]) => value > 0) // Исключаем нулевые значения
\t\t\t.map(([unit, value]) => `\${value} \${unit}\${value > 1 ? 's' : ''}`) // Добавляем \"s\" для множественного числа
\t\t\t.join(' ');
...
\t\t\t\t\t\$('#comlete-time-text').text(calcTimeByQuesting(checkedValues));
\t\t\t\t\tbreak;
\t\t\t}
\t\t
\t});
...
\t\t\t\t\$('#comlete-time-text').text(time + \" order completion\");
\t\t\t}
\t\t\telse{
