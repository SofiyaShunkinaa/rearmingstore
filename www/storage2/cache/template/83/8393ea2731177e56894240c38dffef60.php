...
class __TwigTemplate_62e663590f5b0e2aa9d085b1c85d2869 extends Template
{
    private $source;
    private $macros = [];
...
\t\t\t// Проходим по каждому чекбоксу и добавляем выбранные значения в массив
\t\t\tcheckboxes.forEach(cb => {
\t\t\tif (cb.checked) {
\t\t\t\tselectedValues.push({
\t\t\t\tid: cb.value, // Значение чекбокса (product_option_value_id)
\t\t\t\tprice: cb.getAttribute('data-prise'), // Цена, получаемая из data-атрибута
\t\t\t\tname: cb.getAttribute('data-name') 
\t\t\t\t});
\t\t\t\tconsole.log(cb.getAttribute('data-name') )
\t\t\t}
\t\t\t});
\t\t\t
\t\t});
