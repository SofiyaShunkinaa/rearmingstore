...
class __TwigTemplate_978a51986d27b8836bf6c9178e7330d9 extends Template
{
    private $source;
    private $macros = [];
...
\tvar hiddenSummary = 0;
 \$('#product input[type=\"hidden\"]').each(function() {
    ...
    if (\$(this).attr('data-prise') !== undefined) {
        ...
        // Найдите элемент с классом .option-price внутри текущего элемента
        var prefixElement = \$(this).closest('.row').find('.option-price');
        var prefix = prefixElement.attr('data-prefix'); // Получаем префикс

        result = getNum(price) * value;

        // Используем префикс для определения знака
        if (prefix == '-') {
            result = -result;
        }
