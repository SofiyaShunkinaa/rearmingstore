...
class __TwigTemplate_dbb42edf615489d8786a06e2dc0cdfc1 extends Template
{
    private $source;
    private $macros = [];
...
\t\t}, 0);
\t\t
\t\tif(count==4 || count==2&&values.includes(\"add 1800 rating in arena (free selfplay)\")&&values.includes(\"BiS PvP Gear for Current Meta\"){
\t\t\ttotalWeight--;
\t\t}
...
\t\t} else{
\t\t\ttext = (months>0?months+\" monthes  \":\"\")+(days>0?days+\" days\":\"\")+\" order completion\";
\t\t}
...
\t\t\t\t\"Conquest Gear Boost\", \"Wrathion Reputation Boost\"].includes(productName):
\t\t\t\t\tcalcTimeByEqualExtraOptions(checkedValues, productName);
\t\t\t\t\tbreak;
\t\t\t\tcase [\"Full PvP Gear Boost\"].includes(productName):
\t\t\t\t\tconsole.log(555)
