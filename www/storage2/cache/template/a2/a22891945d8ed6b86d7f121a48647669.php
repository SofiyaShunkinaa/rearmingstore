...
class __TwigTemplate_cc3e7aeaead0029c66d823b4eb893bcc extends Template
{
    private $source;
    private $macros = [];
    ...
    if (\$('#slider').length > 0) {
        ...
        function updateCompletionText(value) {
            ...
            if (document.querySelector('.titlepage').textContent === \"WoW Solo Shuffle Wins Boost\") {
                ...
                // Удаляем символ валюты перед расчетом
...
\t\t\t\t\t// Если изначальная цена не сохранена, сохранить её
...
function calcPrice(){
    ...
    \$('#product input[type=\"radio\"]').each(function(){
        ...
        if(\$(this).prop('checked')){
            var price = parseFloat(\$(this).attr('data-prise')) || 0;
            result = getNum(price);
        }
        all_price += result;
    });
    ...
    all_price = (all_price * count).toFixed(2);

    console.log(\"Total Price: \" + all_price);
