...
class __TwigTemplate_c258c262eefee7e873568d5b293713b3 extends Template
{
    private $source;
    private $macros = [];
...
\t\tvar currentR = \$('.dropdown-item[data-name=\"Current rating\"]').first().data('value');
...
\t\t\tvar hours =\"\";
\t\t\tif(desiredR<1900){
\t\t\t\tlet deltaTimes = (desiredR - currentR)/100;
\t\t\t\tlet hoursTotal = deltaTimes*6;
\t\t\t\tdays = Math.floor(completionTime / 24);
\t\t\t\thours = completionTime % 24;
\t\t\t} else {
\t\t\t\tlet deltaTimes1 = (1800 - currentR)/100;
\t\t\t\tlet deltaTimes2 = (desiredR - 1800)/100;
\t\t\t\tlet hoursTotal = deltaTimes*6;
\t\t\t\tdays = Math.floor(completionTime / 24);
\t\t\t\thours = completionTime % 24;
\t\t\t}
\t\t\t\$('#comlete-time-text').text((days === 0 ? \"\" : (days + \" day \")) + (hours === 0 ? \"\" : (hours + \" hour\" + (hours !== 1 ? \"s\" : \"\"))));
\t\t})
    });
