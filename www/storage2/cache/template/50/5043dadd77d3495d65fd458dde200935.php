...
class __TwigTemplate_0359d27ed423e55fb974057cfb033c69 extends Template
{
    private $source;
    private $macros = [];
...
\t\tvar additionals = 0;
\t\tif (values.some(value => value.includes('PvP ilvl'))) {
\t\t\tadditionals += 2;
\t\t}
\t\tif (values.some(value => value.includes('Express'))) {
\t\t\tadditionals += 0;
\t\t}
\t\tif (values.some(value => value.includes('Full Conquest'))) {
\t\t\tadditionals += 5;
\t\t}
\t\tif (values.some(value => value.includes('PvP Gear'))) {
\t\t\tadditionals += 5;
\t\t}

\t\t\$('#comlete-time-text').attr('data-additionals', additionals);
\t\tconsole.log(\"additionals: \" + additionals + \"  days: \" + days + \" hours: \" + hours);

\t\t// Убедитесь, что вы правильно обновляете days
\t\tdays = Number(days) + Number(additionals);
...
\t\t\tconsole.log(\$('#comlete-time-text').attr('data-additionals'))
\t\t\tlet additional_days = \$('#comlete-time-text').attr('data-additionals') == \"\" ? \"\" : \$('#comlete-time-text').attr('data-additionals');
\t\t\tconsole.log(\"additionals: \"+ additional_days+\"  days: \"+days)
\t\t\tdays = Number(days) + additional_days;
